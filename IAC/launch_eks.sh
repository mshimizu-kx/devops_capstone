...
single_scripts/create_stack.sh ${SERVER_STACKNAME} kdbhdb_server.yml kdbhdb_server_parameter.json --parameters ParameterKey=SSHKey,ParameterValue=udacity.pem

# Wait for server stack to be completed
echo "Waiting until server-stack build completion..."
...
echo "Get Node Role ARN from built stack..."
...
cp ../manifest/aws-auth-cm.yaml aws-auth-cm.yml
sed -i "s/<NODE_ROLE_ARN>/${NODE_ROLE_ARN}/g" aws-auth-cm.yml
...
echo "Finished lauching appication."
